...
%which granules?
Settings.Date = datenum(2005,7,8);
Settings.Gs   = [85:87];
...
Settings.Plotting.ZLevel = 45; %km

%latitude and longitude box
Settings.Plotting.LatRange = [40,65];
Settings.Plotting.LonRange = [-110,-70];

%cross-track row to plot
Settings.Plotting.XTRow = (1:1:60) .*Settings.InterpFactor(1);

%along track elements to plot
Settings.Plotting.ATEls = 116 .*Settings.InterpFactor(1);
...
for iP=1:1:20;
  ...
  switch iP
    case  {1,11}; PlotData = Data.IN;  %Colours(11:21,:) = 1;
...
% %     end
  ...
  elseif strcmp(Type,'s') %slice through data
    ...
    x = Settings.Plotting.XTRow - min(Settings.Plotting.XTRow);
    x = x.*Spacing(1);
