...
%
%modified from the original to use Neil's new 2D+1 code
%
% Corwin Wright, c.wright@bath.ac.uk, 2021/1/28
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%output file
Settings.OutFile = 'out_incfrac.mat';
...
Settings.Wave.Amplitude = [5,0.25:0.25:20];  %K
Settings.Wave.Lambdax   = [500,30:5:100,150:50:1000]; %km - across track
Settings.Wave.Lambday   = [500,30:5:100,150:50:1000]; %km - along track
Settings.Wave.Lambdaz   = [25,1:1:150];  %km - vertical
Settings.Wave.Rotationx = [0:5:170];     %degrees in x to rotate wave
Settings.Wave.Rotationy = [0:5:170];     %degrees in y to rotate wave
Settings.Wave.Rotationz = [0:5:170];     %degrees in z to rotate wave
...
for iVar=1:1:numel(Vars)
  ...
  Results = NaN(2,2,                      ...  %first 2 is flat vs realistic, second 2 is 3D vs 2D+1
                ...
                numel(ToVary));
  FracResults = Results; %results scaled to input variable
  ...
  for iRun=1:1:numel(ToVary)
    ...
    for Flat=[1,2];
      ...
      Bad = find(1./ST.F3_2dp1 > Settings.CutOff.kz);
      ...
      for iAverage =1:1:numel(Settings.Averages)
        ...
        %scale output variables to their equivalent inputs
        switch Settings.OutVars{iOutVar}
          case 'A';  Factor = Params.Amplitude;
          case 'F3'; Factor = 1./Params.Lambdaz;
          case 'kh'; Factor = 1./quadadd(Params.Lambdax,Params.Lambday);
        end
        FracA = VarA./Factor; FracB = VarB./Factor;
        ...
        Results(Flat,2,iOutVar,iAverage,iRun) = abs(fh(VarB(Settings.AvZone.X, ...
                                                            Settings.AvZone.Y, ...
                                                            Settings.AvZone.Z),'all'));  
        FracResults(Flat,1,iOutVar,iAverage,iRun) = abs(fh(FracA(Settings.AvZone.X, ...
                                                            Settings.AvZone.Y, ...
                                                            Settings.AvZone.Z),'all'));
        FracResults(Flat,2,iOutVar,iAverage,iRun) = abs(fh(FracB(Settings.AvZone.X, ...
                                                            Settings.AvZone.Y, ...
                                                            Settings.AvZone.Z),'all'));                                           
        end; clear VarA VarB FracA FracB Factor
      end;
  ...
  AllFracResults.(Vars{iVar}) = FracResults;
  save(Settings.OutFile,'Settings','AllResults','AllFracResults')
